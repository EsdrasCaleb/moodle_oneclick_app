...
### **2. Install using “Deploy from Git” (classic method)**

Prerequisites:
- A running CapRover server.
- A working database (PostgreSQL or MySQL/MariaDB) — can be installed via CapRover One-Click Apps.

Steps:

1. Create a new CapRover app (example: `mymoodle`).
2. Go to the **Deployment** tab → *Method 3: Deploy from GitHub*.
...
The first boot will fail or restart. This is normal — the app needs environment variables.
...
- If directory exists → `git pull` (+ checkout branch if defined)
...
- `/var/www/html` (Moodle core) is **not persisted** — updates come from Git or image tag.  
